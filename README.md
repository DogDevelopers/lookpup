...
| **`review/`** | 리팩토링·성능·보안 검토 기록을 마크다운으로 남겨 근거 추적 |
| **GitHub PR 템플릿** | `.github/PULL_REQUEST_TEMPLATE.md` |
| **ESLint + `tsc --noEmit`** | 커밋 전 통과 기준 |

## 🚀 구 버전 대비 개선 사항

> 상세 근거는 [`review/20260806_legacy-comparison.md`](./review/20260806_legacy-comparison.md) 참고.
> 별개 저장소라 diff가 아닌 **파일·패턴 전수 조사**로 비교했습니다.
...
- **PortOne 웹훅 라우트 부재** — API Route를 전부 걷어내며 함께 사라졌습니다.
  카드 결제는 `features/payments/actions.ts`에서 PortOne API 재조회로 검증하므로 문제없지만,
  `createPayment`가 여전히 받는 `VIRTUAL_ACCOUNT` / `TRANSFER`는 입금이 나중에 일어나므로
  **웹훅 없이는 `payments.status`가 `ready`에 머뭅니다.**
  → 웹훅 라우트 복원(멱등 처리 필수) 또는 `CARD`만 허용, 둘 중 하나가 필요합니다.
- **OG 이미지** — 현재 `public/lookpup_logo.png`를 임시 사용 중입니다.
  1200×630 전용 카드로 교체가 필요합니다.
- **메모이제이션** — `useMemo` / `useCallback` 사용 파일이 91 → 121로 늘었습니다.
  React 19 기준 습관적 메모이제이션은 정리 대상입니다.
