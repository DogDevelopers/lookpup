...
import { X, CheckCircle, CalendarDays, PawPrint, PlayCircle } from "lucide-react";
      ...
      >
        <div className="px-6 pt-6 pb-4 border-b border-orange-100 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-base font-bold text-stone-900">
              {variant === "start" ? "서비스 시작 알림 전송" : "서비스 완료 전송"}
            </p>
          </div>
          <p className="text-gray-500 text-xs mt-0.5 ml-10">
            {variant === "start" ? "시작할 예약을 선택해주세요." : "완료할 예약을 선택해주세요."}
          </p>
        </div>
        ...
        {variant === "start" && (
          <div className="mx-6 mt-4 bg-amber-50 rounded-xl px-4 py-3 text-amber-700 text-xs leading-relaxed shrink-0">
            ...
            <br />
            서비스가 완료되면 반드시 <span className="font-semibold">서비스 완료</span>를 꼭 눌러주세요.
          </div>
        )}
          ...
          {loading ? (
            ...
            <div className="py-10 text-center">
              <p className="text-sm text-stone-400">
                {variant === "start" ? "시작 가능한 예약이 없습니다." : "진행 중인 예약이 없습니다."}
              </p>
            ...
            <div className="flex flex-col gap-3">
              {reservations.map((r) => {
                const statusLabel =
                  variant === "start" ? (STATUS_LABEL_START[r.status] ?? r.status) : (STATUS_LABEL_COMPLETE[r.status] ?? r.status);
                const statusClass =
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-2xl border-2 transition-colors ${
                      selectedId === r.id ? "border-orange-500 bg-orange-50" : "border-orange-100 bg-white hover:border-orange-300"
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <span className="text-sm font-semibold text-stone-900 flex-1">{r.serviceTitle}</span>
                    ...
                    {r.petName && (
                      <div className="flex items-center gap-1.5 text-xs text-gray-500 mb-1">
                        <PawPrint size={11} className="text-orange-400 shrink-0" />
                        <span>{r.petName}</span>
                      </div>
                    )}
                    ...
                    <div className="mt-2 text-xs font-medium text-orange-500">{r.totalPrice.toLocaleString("ko-KR")}원</div>

                    {selectedId === r.id && (
                      <div className="mt-2 flex items-center gap-1">
                        <CheckCircle size={12} className="text-orange-500" />
                        <span className="text-xs text-orange-500 font-medium">선택됨</span>
                      </div>
                    )}
                  </button>
                );
        ...
        <div className="px-6 py-4 flex gap-3 shrink-0">
          ...
          >
            {sending ? "전송 중..." : variant === "start" ? "서비스 시작 알림 전송" : "서비스 완료 전송"}
          </button>
        </div>
      </div>
    </div>
