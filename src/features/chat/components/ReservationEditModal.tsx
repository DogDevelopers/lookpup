    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-orange-100 shrink-0">
          ...
          <button onClick={onClose} className="p-1 rounded-full hover:bg-orange-50 transition-colors">
          ...
          {(["select", "edit", "confirm"] as Step[]).map((s, i) => (
            <div
              ...
              className={`h-1 flex-1 rounded-full transition-colors ${
                (step === "select" && i === 0) || (step === "edit" && i <= 1) || step === "confirm"
                  ? "bg-orange-500"
                  : "bg-orange-100"
              }`}
            />
        ...
        {step === "select" && (
          <div className="px-5 py-4 overflow-y-auto">
            {loading && <p className="text-center text-stone-400 text-sm py-8">불러오는 중...</p>}
            {!loading && fetchError && <p className="text-center text-red-500 text-sm py-8">{fetchError}</p>}
            {!loading && !fetchError && reservations.length === 0 && (
              <p className="text-center text-stone-400 text-sm py-8">수정 가능한 예약이 없습니다.</p>
            )}
            <div className="flex flex-col gap-3">
              {reservations.map((r) => {
                const statusInfo = STATUS_LABELS[r.status] ?? { label: r.status, className: "bg-stone-50 text-stone-500" };
                return (
                  <button
                    key={r.id}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-stone-400">{r.pets.map((p) => p.name).join(", ")}</span>
                    </div>
        ...
        {step === "edit" && selected && (
          <div className="px-5 py-4 overflow-y-auto flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1.5">시작 시간</label>
                <input
                  type="time"
                  value={startTime}
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1.5">종료 시간</label>
                <input
                  type="time"
                  value={endTime}
        ...
        {step === "confirm" && selected && proposed && (
          <div className="px-5 py-4 overflow-y-auto flex flex-col gap-4">
            <p className="text-sm text-stone-500">아래 변경사항을 상대방에게 전송합니다. 상대방이 확인하면 예약이 수정됩니다.</p>
            <div className="border border-orange-100 rounded-xl overflow-hidden">
              ...
              <div className="px-4 py-3 flex flex-col gap-3">
                <DiffRow label="시작 일시" original={fmtDt(selected.start_datetime)} proposed={fmtDt(proposed.start_datetime)} />
                <DiffRow label="종료 일시" original={fmtDt(selected.end_datetime)} proposed={fmtDt(proposed.end_datetime)} />
                <DiffRow label="메모" original={selected.memo || "(없음)"} proposed={proposed.memo || "(없음)"} />
              </div>
            </div>

            {dateRange?.from && (
              <p className="text-xs text-stone-400 text-center">{fmtRange(dateRange.from, dateRange.to, startTime, endTime)}</p>
            )}
...
function DiffRow({ label, original, proposed }: { label: string; original: string; proposed: string }) {
