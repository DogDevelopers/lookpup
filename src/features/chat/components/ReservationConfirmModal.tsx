...
function RowView({ label, icon, value }: { label: string; icon: React.ReactNode; value: string }) {
  return (
    <div className="flex items-start gap-3 py-3 border-b border-orange-50 last:border-0">
      <div className="w-8 h-8 rounded-xl bg-orange-50 flex items-center justify-center shrink-0 mt-0.5">{icon}</div>
...
function RowEdit({ label, icon, children }: { label: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3 py-3 border-b border-orange-50 last:border-0">
      <div className="w-8 h-8 rounded-xl bg-orange-50 flex items-center justify-center shrink-0 mt-0.5">{icon}</div>
      <div className="flex-1 min-w-0">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-2">
          {loading ? (
            ...
            <>
              <RowEdit label="시작 일시" icon={<CalendarDays size={16} className="text-orange-400" />}>
                <input
                  type="datetime-local"
                  value={startDt}
              ...
              <RowEdit label="종료 일시" icon={<CalendarDays size={16} className="text-orange-400" />}>
                <input
                  type="datetime-local"
                  value={endDt}
              ...
              <RowEdit label="위치" icon={<MapPin size={16} className="text-orange-400" />}>
                <input
                  type="text"
                  value={location}
              ...
              <RowEdit label="금액" icon={<CircleDollarSign size={16} className="text-orange-400" />}>
            ...
            <>
              <RowView label="구인글 제목" icon={<ClipboardListIcon />} value={details.title} />
              <RowView
                label="서비스 종류"
                icon={<Wrench size={16} className="text-orange-400" />}
                value={SERVICE_LABEL[details.requestType ?? ""] ?? details.requestType ?? "미정"}
              />
              <RowView label="반려동물" icon={<PawPrint size={16} className="text-orange-400" />} value={petLabel} />
              <RowView
                label="시작 일시"
