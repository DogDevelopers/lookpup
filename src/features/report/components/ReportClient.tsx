...
import { AlertTriangle, Check, ImagePlus, X, Search, User } from "lucide-react";
...
import { createReport, searchReportableUsers, type ReportableUser } from "@/features/report/actions";
import { uploadToCloudinary } from "@/lib/cloudinary";

const REPORT_REASONS = ["부적절한 언행", "허위 정보", "예약 불이행", "반려동물 학대 의심", "사기 의심", "기타"];
...
export default function ReportClient() {
  ...
  const [searchResults, setSearchResults] = useState<ReportableUser[]>([]);
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    setIsSearching(true);
    try {
      const result = await searchReportableUsers(query);
  ...
  const handleSelectUser = (u: ReportableUser) => {
    setTargetId(u.id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const image_urls = await Promise.all(images.map((file) => uploadToCloudinary(file, "reports")));
  ...
  const roleLabel = (role: string | null) => (role === "both" || role === "admin" ? "펫시터" : "");
            ...
            <>
              <SectionCard className="p-6 gap-0">
                <p className="text-lg font-bold text-stone-900">
                  신고 사유<span className="text-red-500 ml-0.5">*</span>
                </p>
