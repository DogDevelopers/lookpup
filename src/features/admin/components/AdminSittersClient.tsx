...
export default function AdminSittersClient({
  ...
  const [sitters, setSitters] = useState<SitterApplication[]>(initialSitters);
  ...
  const getUser = (s: SitterApplication): SitterApplicantUser | null => {
    if (!s.users) return null;
    return Array.isArray(s.users) ? (s.users[0] ?? null) : s.users;
  };

  const handleStatusChange = async (sitter: SitterApplication, newStatus: SitterStatus) => {
