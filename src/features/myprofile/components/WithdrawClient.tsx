...
export default function WithdrawClient() {
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteUser(selectedReason, selectedReason === "other" ? otherText : undefined);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="w-full max-w-[1280px] mx-auto px-4 sm:px-10 pt-4 md:pt-12 pb-20">
        ...
        {done ? (
          <SuccessScreen onGoHome={() => signOut()} />
        ) : (
                      ...
                      >
                        ...
                        <span
                          className={`text-sm font-medium ${
                            selectedReason === r.id ? "text-[var(--color-orange-500)]" : "text-stone-900"
                          }`}
                        >
                          {r.label}
                        </span>
