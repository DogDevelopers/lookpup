      ...
      <div className="p-5 space-y-4">
        <div className="flex gap-4">
          <div className="relative shrink-0">
            <Avatar initial={profile.initial} src={profile.src} size="lg" variant="orange" />
          </div>

          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-lg font-bold text-stone-900">{profile.name}</span>
              {isSitterCard ? (
            ...
            <div className="flex items-center gap-1 mb-1">
              ...
              <span className={`text-xs ${profile.location ? "text-gray-500" : "text-gray-300"}`}>
                {profile.location || "위치 미등록"}
              </span>
            </div>
            {isSitterCard && (
              <div className="flex items-center gap-1">
                <Star size={12} className="fill-amber-400 text-amber-400" />
                <span className="text-sm font-bold text-stone-900">{profile.rating ?? "-"}</span>
                <span className="text-xs text-gray-400">({profile.reviewCount ?? 0}개 리뷰)</span>
              </div>
            )}
          </div>
        ...
        {isSitterCard && !!profile.services?.length && (
          <div className="flex gap-2 flex-wrap">
            {[...new Set(profile.services)].map((s) => (
              <Pill key={s}>{s}</Pill>
            ))}
          </div>
        )}
